...
class MyButton(tk.Button):
    def __init__(self, master, x, y, number=0, *args, **kwargs):
        super(MyButton, self). __init__(master, width=3, font='Times New Roman 15 bold', *args, **kwargs)
        self.x = x
        self.y = y
        self.number = number
        self.is_mine = False
...
class MineSweeper:
    ...
    # Открытие соседних пустых клеток
    ...
    def create_settings_win(self):
        ...
        save_btn = tk.Button(win_settings, text='Сохранить', command=lambda: self.change_settings(row_entry, column_entry, mines_entry))
    ...
    def insert_mines(self, number: int):
        index_mines = self.get_mines_placer(number)
        print(index_mines)
        for i in range(1, MineSweeper.ROW+1):
            for j in range(1, MineSweeper.COLUMNS+1):
                btn = self.buttons[i][j]
    ...
    @staticmethod
    def get_mines_placer(exclude_number: int):
